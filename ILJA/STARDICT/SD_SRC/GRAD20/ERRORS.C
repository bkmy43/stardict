 ...
 switch ( code )
  {
   ...
   case CAN_NOT_OPEN_KEY    :
      strcpy( UserMessage, "Ошибка при открытии файла keyword.tmp");
      ErrorType=CRITICAL;
      break;
   case NO_IN_NAME    :
   ...
   case ERROR_IN_TH          :
      ...
      break;
   case ERROR_IN_KEYS          :
      strcpy( UserMessage, "Файл ключевых слов испорчен");
      ErrorType=CRITICAL;
      break;
   ...
   case TOO_MANY_KEYWORDS_IN_ART :
      strcpy( UserMessage, "Количество кейвордов в статье больше предельно допустимого");
      ErrorType=CRITICAL;
      break;

   case TOO_MANY_KEYWORDS_IN_LINK :
      strcpy( UserMessage, "Количество кейвордов внутри ссылки больше предельно допустимого");
      ErrorType=CRITICAL;
      break;

   case WRONG_KEYWORD :
      strcpy( UserMessage, "Ошибка в ключевом слове");
      ErrorType=CRITICAL;
      break;
...
// записать в протокол.
  ...
  fprintf( erlist, "В статье (теме) номер %d\n"
		 "Под названием: \"%s\"\n"
		 "При обработке %d строки исходного файла\n"
		 "ОБНАРУЖЕНА ОШИБКА: %s.\n"
		  "----------------------------------------------------------------------\n\n",
