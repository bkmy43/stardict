...
int DelFirstString=OFF;
 // режим, при котором первая строка удаляется. Нужен для совместимости со
 // старыми исходными файлами в которых в первой строке стояло название
 // статьи. Версия STARDICT 2.0 автоматически подставляет это название
 // в caption окна отображающего статью.
...
int main( int argc, char* argv[] )
   {
      ...
      if( WorkMode == HELP )
       {
	 printf(
	     "  Программа выполняет перекодировку исходных текстов астрологической\n"
	     "  энциклопедии Stardict из формата STEXT (1.0) в формат RSTEXT (2.0).\n\n"
	     "   ВЕРСИЯ       1 от 15.10.97\n\n"
	     "   Синтаксис:   CONV220 [-t] [-f] <infile> <outfile>\n\n"
	     "   Параметры:   -t  режим чтения тем\n"
	     "                -f  режим удаления первой строки текста статьи\n\n");
	 return OK;
       }

      printf( "Перекодировка исходных текстов астрологической энциклопедии Stardict\n"
 ...
 //     SaveChanges();
      ...
      printf("Конвертация завершена... Гарантий нет. :-)\n\n");
      if( WorkMode == ART_READ )
...
void SetWorkMode(int argc, char* argv[])
{
	 ...
	 if( strcmp( argv[i],"-f" ) == 0 || strcmp( argv[i],"-F" ) == 0)
	    DelFirstString=ON; // режим прверки CRC  dat-файлов
       }
