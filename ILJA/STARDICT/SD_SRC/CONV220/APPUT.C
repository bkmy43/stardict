...
char UPPER_SYMBOLS[]={"АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯABCDEFGHIJKLMNOPQRSTUVWXYZ"};
char LOWER_SYMBOLS[]={"абвгдеёжзийклмнопрстуфхцчшщъыьэюяabcdefghijklmnopqrstuvwxyz"};
...
void LowCase( char* str) // Ф-я конвертирует все символы кроме первого в lower case
{
...
void LowCase( char& ch) // Ф-я конвертирует символ в lower case а-я, a-z
{
 for(int j=0; UPPER_SYMBOLS[j]!=0 && UPPER_SYMBOLS[j]!=ch; j++ );

 if( UPPER_SYMBOLS[j]!=0 )    // если ch - большая буква
     ch = LOWER_SYMBOLS[j];
}
